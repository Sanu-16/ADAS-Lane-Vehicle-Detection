...
_cal_data_loaded = False
_mtx = None
_dist = None

def undistort(img, cal_dir='cal_pickle.p'):
    global _cal_data_loaded, _mtx, _dist
    if not _cal_data_loaded:
        _cal_data_loaded = True
        if os.path.isfile(cal_dir):
            try:
                ...
                print(f"[!] Warning: Could not read calibration file {cal_dir}: {e}")
        else:
            print(f"[!] Note: Calibration file '{cal_dir}' not found. Skipping distortion correction.")
...
def colorFilter(img):
    ...
    # Mask out top portion (sky area) to avoid false white triggers
    h, w = img.shape[:2]
    sky_mask = np.zeros_like(combinedImage)
    sky_mask[int(h * 0.40):, :] = 255
    combinedImage = cv2.bitwise_and(combinedImage, sky_mask)
...
def drawHUD(img, lane_curve, fps=0.0):
    h, w = img.shape[:2]
    overlay = img.copy()
    
    # Top semi-transparent banner
    cv2.rectangle(overlay, (0, 0), (w, 65), (15, 15, 25), -1)
    # Bottom controls legend bar
    cv2.rectangle(overlay, (0, h - 25), (w, h), (15, 15, 25), -1)
    
    img = cv2.addWeighted(overlay, 0.70, img, 0.30, 0)
    
    # Determine Steering & Lane Drift based on road curvature
    curve_val = int(lane_curve)
    abs_curve = abs(curve_val)
    
    if abs_curve > 1200 or abs_curve == 0:
        direction = "STRAIGHT"
        curve_str = "INF"
        dir_color = (0, 255, 128)
        ldw_text = "SAFE - IN LANE"
        ldw_color = (0, 255, 0)
    elif curve_val > 0:
        curve_str = f"{curve_val}m"
        if curve_val < 300:
            ...
            dir_color = (0, 165, 255)
            ldw_text = "WARNING: DRIFTING"
            ldw_color = (0, 0, 255)
    else:
        curve_str = f"{abs_curve}m"
        if abs_curve < 300:
    ...
    # Col 1 (Left): Performance & Curve
    cv2.putText(img, f"FPS: {fps:.1f}", (15, 25), font, 0.55, (255, 255, 255), 1, cv2.LINE_AA)
    cv2.putText(img, f"CURVE: {curve_str}", (15, 50), font, 0.55, (200, 200, 200), 1, cv2.LINE_AA)
    
    # Col 2 (Center): Steering Direction & Safety Status
    cv2.putText(img, f"STEER: {direction}", (160, 25), font, 0.60, dir_color, 2, cv2.LINE_AA)
    cv2.putText(img, f"STATUS: {ldw_text}", (160, 50), font, 0.55, ldw_color, 2, cv2.LINE_AA)
    
    # Col 3 (Right): Quick Help Key Legend
    cv2.putText(img, "[P] Pause  [S] Save  [Q] Quit", (w - 230, 25), font, 0.48, (180, 220, 255), 1, cv2.LINE_AA)
    cv2.putText(img, "ADAS Detection System", (w - 230, 50), font, 0.50, (255, 255, 255), 1, cv2.LINE_AA)
    
    # Bottom Legend Bar
    cv2.putText(img, "Press [P] to pause video | [S] to save screenshot | [Q] to exit", (15, h - 7), font, 0.45, (180, 180, 180), 1, cv2.LINE_AA)
    
    return img


def drawFancyBoundingBox(img, x, y, w_box, h_box, label, color):
    # Main bounding rectangle
    cv2.rectangle(img, (x, y), (x + w_box, y + h_box), color, 2)
    
    # Corner accent lines for modern HUD look
    length = int(min(w_box, h_box) * 0.2)
    t = 3
    # Top-Left
    cv2.line(img, (x, y), (x + length, y), color, t)
    cv2.line(img, (x, y), (x, y + length), color, t)
    # Top-Right
    cv2.line(img, (x + w_box, y), (x + w_box - length, y), color, t)
    cv2.line(img, (x + w_box, y), (x + w_box, y + length), color, t)
    # Bottom-Left
    cv2.line(img, (x, y + h_box), (x + length, y + h_box), color, t)
    cv2.line(img, (x, y + h_box), (x, y + h_box - length), color, t)
    # Bottom-Right
    cv2.line(img, (x + w_box, y + h_box), (x + w_box - length, y + h_box), color, t)
    cv2.line(img, (x + w_box, y + h_box), (x + w_box, y + h_box - length), color, t)
    ...
    tag_y2 = max(text_h + 8, y)
    cv2.rectangle(img, (x, tag_y1), (x + text_w + 10, tag_y2), color, -1)
    cv2.putText(img, label, (x + 5, tag_y2 - 5), font, font_scale, (255, 255, 255), thickness, cv2.LINE_AA)
